...
- DataSourceConfig.kt 파일을 다음과 같이 수정한다. 
...
### mybatis 설정 및 mapper 설정하기

- resources/mybatis-config.xml 파일을 다음과 같이 변경한다. 
...
<mapper namespace="com.schooldevops.springbatch.batchsample.jobs">
    ...
    <resultMap id="customerResult" type="com.schooldevops.spring_batch.jobs.data.Customer2">
    ...
    <select id="selectCursorCustomers" resultMap="customerResult" fetchSize="2">
        SELECT id, name, age, gender
        FROM customer
    </select>
</mapper>
...
## CustomerItemProcessor 작성하기

- 필수는 아니지만 이번에는 CustomerItemProcessor.kt파일을 다음과 같이 작성해보자. 
- 단순히 로깅을 수행하는 ItemProcessor이다. 
